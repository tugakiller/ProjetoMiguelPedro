...
<body>
    <?php
        ...
        echo "<br>".$whatIWant."<br>";
        
        $con = mysqli_connect("localhost","root","", "p_l_m");
        
        //Select do servidor escolhido
        $sql = 'SELECT id, Tipo, numero_de_perticipantes, Valor_em_mesa FROM server_list WHERE id='.$whatIWant.'';
        //Select do servidor jogavel
        $sql2='SELECT id, id_server_list, id_pessoa FROM server_jogavel WHERE id_server_list='.$whatIWant.'';
        //Select dos jogadores no server
        $sql3='SELECT pessoa.id, nome, email, valor FROM pessoa, server_jogavel WHERE server_jogavel.id_pessoa = pessoa.id';
